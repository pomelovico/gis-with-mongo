...
function insertShpToDB($shp,$dbf){
	global $infoType;
	require_once('../lib/shapefile.php');
	$tempfile_name = $shp['tmp_name'];
    $upload_file_name = $shp['name'];
    print_r($shp);
    print_r($dbf);
    try{
    	$ShapeFile = new ShapeFile($shp['tmp_name'],$dbf['tmp_name']);
