...
/*        print_r(iconv_get_encoding());
        echo 'encode type:<br>'.mb_detect_encoding($rawData, array("ASCII",'UTF-8',"GB2312","GBK",'BIG5'));
        echo '<hr>In XBaseRecord: <br> rawData:'.iconv("GB2312","UTF-8//IGNORE",$rawData).'<hr>';
        echo '$table:<br>'.json_encode($table->getColumns())."<hr>";
        echo 'substr: '.iconv("GB2312","UTF-8//IGNORE",substr($rawData,114,50))."<hr>";*/
        $this->choppedData = array();
        if ($rawData && strlen($rawData)>0) {
	        $this->inserted=false;
        	$this->deleted=(ord($rawData[0])!="32");
        	foreach ($table->getColumns() as $column) {
/*                echo 'pos: '.$column->getBytePos();
                echo ', len: '.$column->getDataLength()."<hr>";*/
                $this->choppedData[]=iconv("GB2312","UTF-8//IGNORE",substr($rawData,$column->getBytePos(),$column->getDataLength()));
            	// $this->choppedData[]=substr($rawData,74,32);
        	}
/*            echo $this->choppedData[6].'<hr>';*/
    	} else {
	    	$this->inserted=true;
	    	$this->deleted=false;
	    	foreach ($table->getColumns() as $column) {
    ...
    function forceGetString($columnObj) {
        // echo 'In Record:'.$this->choppedData[3].'<br>';
        if (ord($this->choppedData[$columnObj->getColIndex()][0])=="0") return '';
    ...
    function getDate($columnObj) {
        try{
            if ($columnObj->getType()!=DBFFIELD_TYPE_DATE)
                throw new Exception('RECORD_WRONG_DATE');
    ...
    function getDateTime($columnObj) {
        try{
            if ($columnObj->getType()!=DBFFIELD_TYPE_DATETIME)
                throw new Exception("RECORD_WRONG_DATETIME");
    ...
    function setObject($columnObj,$value) {
        ...
        try{
            throw new Exception("RECORD_WRONG_DATATYPE");
        }catch (Exception $e){
            TriggerError($e);
        }
        //trigger_error ("cannot handle datatype".$columnObj->getType(), E_USER_ERROR);
