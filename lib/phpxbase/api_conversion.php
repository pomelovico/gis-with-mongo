...
function xbase_get_record_with_names($xbase_identifier,$record) { // - Gets a record from a dBase database as an associative array 
	if (!($xbase=xbase_getInstance($xbase_identifier))) return false;
	$r =$xbase->moveTo($record-1);
	$result = array();
	$count = 0;
	foreach ($xbase->columnNames as $column) {
		$result[$column] = $r->getString($xbase->columns[$count]);
		// echo $count.$column->type.":".$result[$column->name] ;
		$count++;
	}
...
function &xbase_getInstance($i=NULL) {
	...
	try{
		if (sizeof($xbase_instances)==0)
			throw new Exception('NO_XBASE_AVAILABLE',8004);
		if (is_null($i)) {
			$result =current($xbase_instances);
		} else {
			if (!@$xbase_instances[$i])
				throw new Exception("INVALID_XABSE_IDENTIFIER",8004);
